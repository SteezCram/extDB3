...
Ext::Ext(std::string shared_library_path)
{
	...
	try
	{
		...
		logger->info("extDB3: Version: {0}", EXTDB_VERSION);
			...
			logger->info("");
			logger->info("Message: This version was modified and include some improvement");
			logger->info("Message: The lib was recompiled using new version of TBBMalloc, Boost and MariaDB connector");
			logger->info("Message: The worker threads limitation was removed");
			logger->info("Message: Steez: 30/10/2020");
			logger->info("");
			logger->info("");
		#endif
		...
		else
		{
			...
			// Remove the sanity check, cause it's not useful today
